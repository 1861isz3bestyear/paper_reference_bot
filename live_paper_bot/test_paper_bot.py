...
from live_paper_bot.config import PaperBotConfig
from live_paper_bot.trading import LocalPaperTrader
...
class TestPaperBot:
    ...
    def test_calculate_target_side_uses_offset_and_strategy_launch(self, monkeypatch):
        ...
        monkeypatch.setattr("live_paper_bot.cli.add_launch_weekly_anchored_vwap", indicator)
        monkeypatch.setattr("live_paper_bot.cli.backtest", backtest)
        monkeypatch.setattr("live_paper_bot.cli.latest_strategy_side", latest)
    ...
    def test_stats_use_terminal_friendly_multiline_layout(self) -> None:
        ...
        output = StringIO()
        with (
            patch("live_paper_bot.cli.LocalPaperTrader.trade_history", return_value=[trade]),
            patch("live_paper_bot.cli._configured_market", return_value=("BTCUSDT", "BTC", "USDT")),
            patch("live_paper_bot.cli.shutil.get_terminal_size", return_value=__import__("os").terminal_size((60, 24))),
    ...
    def test_retries_do_not_duplicate_processed_signal(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                ...
                patch(
                    "live_paper_bot.cli.calculate_strategy_decision",
                    side_effect=[StrategyDecision(None, None), StrategyDecision("Long", 102.0)],
                ),
                patch("live_paper_bot.cli.fetch_live_price", return_value=102.0),
                patch("live_paper_bot.cli.load_bybit_credentials", return_value=("key", "secret")),
                patch("live_paper_bot.cli.fetch_bybit_taker_fee_rate", return_value=Decimal("0.00055")),
            ):
    ...
    def test_network_failure_does_not_advance_candle_cursor(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                ...
                patch("live_paper_bot.cli.fetch_live_price", side_effect=requests.ConnectionError("offline")),
            ):
    ...
    def test_catch_up_replays_historical_fills_and_times_without_live_price(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                ...
                patch("live_paper_bot.cli.calculate_strategy_decision", side_effect=decisions),
                patch("live_paper_bot.cli.fetch_live_price") as live_price,
                patch("live_paper_bot.cli.load_bybit_credentials", return_value=("key", "secret")),
                patch(
                    "live_paper_bot.cli.fetch_bybit_taker_fee_rate",
            ...
            with (
                patch.object(bot, "_load_completed_candles", return_value=backlog),
                patch("live_paper_bot.cli.calculate_strategy_decision") as decision,
            ):
    ...
    def test_reversal_sizes_new_position_from_post_close_capital(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                ...
                patch("live_paper_bot.cli.fetch_live_price", return_value=110.0),
                patch("live_paper_bot.cli.load_bybit_credentials", return_value=("key", "secret")),
                patch("live_paper_bot.cli.fetch_bybit_taker_fee_rate", return_value=fee_rate),
            ):
                bot.process_available_candles()
    ...
    def test_anchor_offset_loads_historical_candles_but_does_not_process_them(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                patch("live_paper_bot.cli.fetch_completed_linear_klines", return_value=history) as fetch,
            ):
    ...
    def test_stop_loss_uses_actual_local_fill_and_is_recorded(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                ...
                patch("live_paper_bot.cli.fetch_live_price", return_value=90.0),
                patch("live_paper_bot.cli.load_bybit_credentials", return_value=("key", "secret")),
                patch("live_paper_bot.cli.fetch_bybit_taker_fee_rate", return_value=Decimal("0.00055")),
            ):
                bot.process_available_candles()
    ...
    def test_run_forever_starts_and_stops_websocket(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            output = StringIO()
            with patch("live_paper_bot.cli.signal.signal"), redirect_stdout(output):
                bot.run_forever()
...
def test_status_stats_and_pid_helpers(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr("live_paper_bot.cli.STATE_FILE", state)
    monkeypatch.setattr("live_paper_bot.cli.read_pid", lambda: None)
    monkeypatch.setattr("live_paper_bot.cli.LocalPaperTrader.current_position", Mock(return_value=Mock(side=None, quantity=0, entry_price=None)))
    monkeypatch.setattr("live_paper_bot.cli._configured_market", lambda *_: ("BTCUSDT", "BTC", "USDT"))
    status_command()
    status_output = capsys.readouterr().out
    assert "Paper bot: stopped" in status_output
    assert "Deposit size: 0.00 USDT" in status_output

    monkeypatch.setattr("live_paper_bot.cli.LocalPaperTrader.trade_history", Mock(return_value=[]))
    stats_command()
    assert "No completed" in capsys.readouterr().out

    pid_file = tmp_path / "pid"
    monkeypatch.setattr("live_paper_bot.cli.PID_FILE", pid_file)
...
def test_reset_command_deletes_previous_data_but_keeps_config(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr("live_paper_bot.cli.PAPER_ACCOUNT_FILE", account)
    monkeypatch.setattr("live_paper_bot.cli.STATE_FILE", state)
    monkeypatch.setattr("live_paper_bot.cli.CANDLE_CACHE_FILE", candles)
    monkeypatch.setattr("live_paper_bot.cli.PID_FILE", pid)
    monkeypatch.setattr("live_paper_bot.cli.INSTANCE_LOCK_FILE", instance_lock)
    monkeypatch.setattr("live_paper_bot.cli.read_pid", lambda: None)
...
def test_reset_command_refuses_while_bot_is_running(monkeypatch):
    monkeypatch.setattr("live_paper_bot.cli.read_pid", lambda: 123)
    monkeypatch.setattr("live_paper_bot.cli.process_is_running", lambda _: True)
