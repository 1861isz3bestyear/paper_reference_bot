...
def fetch_bybit_order_limits(symbol: str) -> tuple[Decimal, Decimal, Decimal]:
    ...
    response = requests.get(BYBIT_INSTRUMENTS_URL, params={"category": "linear", "symbol": symbol}, timeout=20)
    response.raise_for_status()
    payload = response.json()
    if payload.get("retCode") != 0:
    ...
    try:
        step = Decimal(str(limits["qtyStep"])); minimum = Decimal(str(limits["minOrderQty"]))
...
class LocalPaperTrader:
    ...
    def build_target(
        ...
        quantity_step: Decimal = BTC_QUANTITY_STEP, minimum_quantity: Decimal = BTC_QUANTITY_STEP,
        ...
        if side is not None and (quantity < minimum_quantity or quantity * Decimal(str(price)) < exchange_minimum_notional):
            ...
            raise ValueError(f"Paper allocation is too small; {symbol} requires at least {minimum_quantity} units and approximately {required} quote notional.")
